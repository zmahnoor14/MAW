...
def main():
    if len(sys.argv) != 1:
        print("Usage python3 combine_specdb.py input_directory")
    else:
...
def combine_specdb(input_dir):
    ...
    if len(GNPScsvfiles2) > 0 and len(HMDBcsvfiles2) > 0 and len(MassBankcsvfiles2) > 0:
        ...
        for i, row in df.iterrows():
            ...
            if (
                os.path.exists(df["MBr"][i])
                and os.path.exists(df["HMDBr"][i])
                and os.path.exists(df["GNPSr"][i])
            ):
                # merge on the basis of Idx
                MergedRE = CSVfileG.merge(CSVfileH, on="id_X").merge(
                    CSVfileM, on="id_X"
                )
    ...
    if (
        len(GNPScsvfiles2) > 0
        and len(HMDBcsvfiles2) == 0
        and len(MassBankcsvfiles2) > 0
    ):
    ...
    if (
        not isNaN(GNPScsvfiles2)
        and not isNaN(HMDBcsvfiles2)
        and isNaN(MassBankcsvfiles2)
    ):
