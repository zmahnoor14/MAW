...
**Note**
<to use MAW with SIRIUS5, follow instructions given at the end> <br>
...
The MAW-R and MAW-Py runs within docker but with the scripts given in MAW/Docker/SIRIUS. The rest of instructions on how to use these R and Python scripts remains the same. So with SIRIUS, there are three docker containers and three steps: 1) Run MAW-R script from MAW/Docker/SIRIUS within the docker container, 2) Run the docker container for SIRIUS, with the instructions given in this section, 3) Run MAW-Py script from MAW/Docker/SIRIUS within the docker container. The instructions to run SIRIUS, are given below.
...
There are changes in MAW-Py that takes SIRIUS as the insilico fragmentation tool. follow the instructions to run MAW-Py with SIRIUS5.
Run docker container for MAW-Py
```
docker run -v $(pwd):/opt/workdir/ --platform linux/amd64 -it zmahnoor/maw-py:1.0.7 /bin/bash
```
Use thw following command within the container: 

```python
python3 Workflow_Python_Script_all_SIRIUS.py --file_id any_id --ms1data file/insilico/MS1DATA.csv --db coconut --msp_file file/spectral_dereplication/spectral_results.csv --gnps_dir file/spectral_dereplication/GNPS --hmdb_dir file/spectral_dereplication/HMDB --mbank_dir file/spectral_dereplication/MassBank
```
remember to mount the pwd and pwd should be the directory which contains the /file result directory that was generated by MAW-R. Also you have make a small change in the following line of MAW-Py script. line numbers 3544; change the path to your json files, it should only be changed before/insilico.
